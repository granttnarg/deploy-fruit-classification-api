        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if track_inference_time and hasattr(result, 'inference_time'):
                    prediction_log["inference_time_seconds"] = round(result.inference_time, 4)
                    prediction_log["preprocessing_time_seconds"] = round(total_time - result.inference_time, 4)
                ...
                input_file = kwargs.get('input_image') or (args[1] if len(args) > 1 else None)
                ...
                getattr(prediction_logger, log_level)(f"PREDICTION: {json.dumps(prediction_log)}")
                return result
